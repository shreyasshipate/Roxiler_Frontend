    ...
    const fetchData = async () => {
      try {
        const pieResponse = await axios.get(
          `https://roxiler-backend-1-jsop.onrender.com/pie-chart?month=${month}`
        );
        const barResponse = await axios.get(
          `https://roxiler-backend-1-jsop.onrender.com/bar-chart?month=${month}`
        );
        const combinedResponse = await axios.get(
          `https://roxiler-backend-1-jsop.onrender.com/combined-data?month=${month}`
        );
